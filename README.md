...
打包成 PWA，支援 Android Chrome 或 iOS Safari「加到主畫面」選項，方便下載成 App 單獨使用。

# 開發環境

- [Bun](https://bun.sh) v1.2.1+ 替代 NodeJS
- [Firebase CLI](https://firebase.google.com/docs/cli?hl=zh-tw) (firebase-tools)
...
- 後端 serverless API 使用 [Firebase Functions](https://firebase.google.com/docs/functions) (Node.js v20)，proxy 要求至第三方天氣服務 API
- [Firebase Hosting](https://firebase.google.com/docs/hosting) 託管前端檔案
...
使用 [Chart.js](https://github.com/chartjs/Chart.js) 與 [Wrapper for React](https://github.com/reactchartjs/react-chartjs-2) 產生氣溫變化圖表，自製 Chart.js plugin 用以在折線圖上繪製天氣 icon。

# 本機開發

## **第三方服務需求**：

- 需登入 Firebase 服務，連結現有或新建專案，並使用 Functions（與 Hosting，選用）功能，參考 [Firebase 指南](https://firebase.google.com/docs/functions/get-started)
...
於根目錄新建一個環境變數檔案 `.env.local` ，並寫入 Google Geolocation API 金鑰與 Firebase 專案 ID：
...
- 於 functions 資料夾使用 `firebase` 工具指令，將 Open Weather Map 提供的 API Key 以名稱欄位 `OPENWEATHER_KEY` 保存在雲端專案：
...
**_已使用 [Bun](https://bun.sh/) 取代 Node.js 與 npm， 嘗試新一代 JS 執行環境(runtime)與套件管理工具(package manager)_**
